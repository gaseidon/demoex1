...
session_start();
	...
	while($row = $result->fetch_assoc()) {
		...
		' : $adv;
		$orders .= sprintf('
			<div class="col col_order text-left main_ord">
				<h2>Заказ %s</h2>
		...
		<div class="content">
			...
			<form enctype="multipart/form-data" action="controllers/add_product.php" method="POST">
				...
				<select name="category" required>
					<option value selected disabled>Категория ㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤ ㅤㅤㅤㅤ▼</option>
					<?= $categories ?>
				</select>
			...
			<form action="controllers/category_delete.php" method="POST">
				<div class="part">
					<select name="category" required>
						...
						<option value selected disabled>Категории ㅤㅤㅤㅤㅤㅤㅤㅤㅤ▼</option>
						<?= $categories ?>
					</select>
			...
			<select id="ord">
					<option value disabled selected>Фильтрация по заказам ㅤㅤㅤㅤㅤ▼</option>
